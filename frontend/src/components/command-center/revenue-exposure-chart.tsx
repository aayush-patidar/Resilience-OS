...
export function RevenueExposureChart() {
  ...
  const totalRevenueAtRisk = (!isRecovered && activeDisruption?.summary?.revenue_at_risk) ? activeDisruption.summary.revenue_at_risk : 0;
  ...
  return (
    <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm p-4 flex flex-col h-[280px]">
      ...
      {activeDisruption && !isRecovered && totalRevenueAtRisk > 0 ? (
        <>
          <div className="mb-2">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center opacity-60">
           ...
           <p className="text-[13px] text-slate-500 font-medium">{isRecovered ? 'All revenue protected' : 'Awaiting simulation data'}</p>
        </div>
      )}
    </div>
  );
